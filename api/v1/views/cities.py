...
def post_city(state_id):
    """Creates a new City object and returns it with status code 201"""
    obj_state = storage.get(State, state_id)
    if not obj_state:
        abort(404)
    ...
    if not isinstance(new_city['name'], str) or not new_city['name'].strip():
...
def put_city(city_id):
    """Updates a City object and returns it with status code 200"""
    obj = storage.get(City, city_id)
    if not obj:
        abort(404)
